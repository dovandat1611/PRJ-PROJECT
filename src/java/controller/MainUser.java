    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("displayAll")) {
                ...
                ResultSet rs = dao.getData("select DISTINCT  category_name "
                        + "from products");
                String title = "List of Products";
                //select JSP

                RequestDispatcher dispath
                        = request.getRequestDispatcher("/jspUser/indexUser.jsp");

                //set object paramter for JSP
                request.setAttribute("data", vector);
                request.setAttribute("tableTitle", title);
                request.setAttribute("data1", rs);
                //Dispatcher to JSP
            ...
            if (service.equals("displayProductdetail")) {
                ...
                Vector<Product> vector = dao.getProduct("select * from products "
                        + "where category_name ='" + cid + "'");
                ResultSet rs = dao.getData("select DISTINCT  category_name "
                        + "from products");

                String title = "List of Products";
                //select JSP
                RequestDispatcher dispath = request.getRequestDispatcher("/jspUser/indexUser.jsp");
                //set object paramter for JSP
                request.setAttribute("data", vector);
                request.setAttribute("tableTitle", title);
                request.setAttribute("data1", rs);
